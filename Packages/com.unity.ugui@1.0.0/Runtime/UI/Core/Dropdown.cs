...
namespace UnityEngine.UI
{
    ...
    {
        protected internal class DropdownItem : MonoBehaviour, IPointerDownHandler, ISelectHandler, ICancelHandler
        {
            [SerializeField]
            private Text m_Text;
            [SerializeField]
            ...
            public class SelectEvent : UnityEvent<RectTransform>
            { }

            public SelectEvent onSelectChanged = new SelectEvent();
            ...
            {
                onSelectChanged.Invoke(rectTransform);
            }
        ...
        public void AutoScroll(RectTransform rectTransform)
        {
            
            float SCROLL_MARGIN = 0f;

            ScrollRect sr = m_Dropdown.GetComponentInChildren<ScrollRect>();

            float contentHeight = sr.content.rect.height;
            float viewportHeight = sr.viewport.rect.height;

            // what bounds must be visible?
            float centerLine = rectTransform.gameObject.transform.localPosition.y; // selected item's center
            float upperBound = centerLine + (rectTransform.rect.height / 2f); // selected item's upper bound
            float lowerBound = centerLine - (rectTransform.rect.height / 2f); // selected item's lower bound
            ...
            if (upperBound > upperVisible)
            {
                ...
                desiredLowerBound = upperBound - viewportHeight + rectTransform.rect.height * SCROLL_MARGIN;
            }
            else if (lowerBound < lowerVisible)
            {
                // need to scroll down to lowerBound
                desiredLowerBound = lowerBound - rectTransform.rect.height * SCROLL_MARGIN;
            }
            else
            {
        ...
        public void Show()
        {
            ...
            for (int i = 0; i < optionsCount; ++i)
            {
                ...
                item.onSelectChanged.AddListener(x => AutoScroll(item.rectTransform));
            ...
            // loop the toggle Dropdown navigation
            if (m_Items.Count > 1)
            {
                Toggle first = m_Items[0].toggle;
                Toggle last = m_Items[m_Items.Count - 1].toggle;
                ...
                // Wrap vertically
                firstNav.selectOnUp = last;
                lastNav.selectOnDown = first;

                // Wrap horizontally (if needed)
                firstNav.selectOnLeft = last;
                lastNav.selectOnRight = first;
            ...
            AutoScroll(m_Items[value].rectTransform);
